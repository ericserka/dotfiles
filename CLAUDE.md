...
- **`fish/CLAUDE.md`** — Fish shell. Autoloading model (`conf.d` vs `functions` vs `completions`), the `tt` time-tracker state machine, Bitwarden/vault/recording helpers, and how to reload/lint (`source`, `fish -n`).
...
- **Time tracker `tt`.** Lives in `fish/` (`functions/tt.fish`, `completions/tt.fish`) but is stopped on shutdown by `systemd/user/tt-auto-stop.service` (`ExecStop=fish -c "tt stop"`). Changing `tt`'s CLI can affect that unit.
